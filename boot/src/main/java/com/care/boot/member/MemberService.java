...
public class MemberService {
    ...
    public String saveFinalMemberInfo(MemberDTO member, Map<String, String> params) {
        // 선택된 이상형 정보 활용 예시 (필요시 DTO에 필드 추가)
        // member.setIdealMbti(params.get("idealOption0"));
        // member.setIdealSmoke(params.get("idealOption1"));
        // 등등...

        // 현재는 기본 회원 정보만 저장하므로 아래와 같이 처리
        int result = mapper.registProc(member);  // 이미 DB에 저장하지 않았다면
